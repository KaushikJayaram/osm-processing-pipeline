-- Simplified Persona Scoring: Phase 1 Implementation - TEST BBOX ONLY
-- 
-- This script implements the simplified framework for TESTING (Karnataka region only):
...
--
-- NOTE: This script only CALCULATES/UPDATES scores for roads in the test bbox
-- For production (all of India), use: 01_compute_persona_base_scores_simplified_all_india.sql
...
WITH test_bbox AS (
    -- Test bounding box: Karnataka region (12-14° lat, 76-78° lon)
    SELECT ST_MakeEnvelope(76, 12, 78, 14, 4326) AS bbox
),
factors AS (
    SELECT
        o.osm_id,
    ...
    CROSS JOIN test_bbox AS b
    WHERE o.bikable_road = TRUE
      AND ST_Intersects(o.geometry, b.bbox)  -- Filter to test bbox
),
raw_scores AS (
    SELECT
        f.*,
        ...
        -- ============================================
        -- MileMuncher: Road Quality (50%) + Straightness (30%) + Low Stress (20%)
        ...
        -- ============================================
        -- CornerCraver: Twistiness (40%) + Road Quality (30%) + Low Stress (20%) + Surface Quality (10%)
        -- Updated: Added road quality (highways are fast, corners are fun when fast)
        -- ============================================
        CASE
            -- Hard gate: Urban roads get 0 score
            WHEN f.road_setting_i1_value = 'Urban' THEN 0.0
        ...
        -- ============================================
        -- TrailBlazer: Exploration (40%) + Scenery (35%) + Low Stress (25%)
        -- Hard gate: Urban roads get 0 score
        -- ============================================
        CASE
            -- Hard gate: Urban roads get 0 score
...
BEGIN
    ...
    RAISE NOTICE 'UPDATE: Updated persona scores for % roads in test bbox', v_update_count;
END $$;

-- Debug: Check if specific way was updated
DO $$
DECLARE
    v_way_id BIGINT := 215233003;
    v_in_normalized INTEGER;
    v_updated_score DOUBLE PRECISION;
BEGIN
    -- Check if way is in normalized_scores CTE
    SELECT COUNT(*) INTO v_in_normalized
    FROM (
        WITH test_bbox AS (
            SELECT ST_MakeEnvelope(76, 12, 78, 14, 4326) AS bbox
        ),
        factors AS (
            ...
            CROSS JOIN test_bbox AS b
            WHERE o.bikable_road = TRUE
              AND ST_Intersects(o.geometry, b.bbox)
              AND o.osm_id = v_way_id
        )
        SELECT osm_id FROM factors
    ) AS check_cte;
    
    -- Get actual stored score
    SELECT persona_cornercraver_base_score INTO v_updated_score
    FROM osm_all_roads
    WHERE osm_id = v_way_id;
    
    RAISE NOTICE 'Debug for way %: In normalized_scores CTE: %, Stored score: %', 
        v_way_id, v_in_normalized, v_updated_score;
    
    IF v_in_normalized = 0 THEN
        RAISE WARNING 'Way % was NOT in normalized_scores CTE - UPDATE did not run!', v_way_id;
    ELSIF v_updated_score IS NULL THEN
        RAISE WARNING 'Way % was in CTE but score is NULL - UPDATE may have failed!', v_way_id;
    ELSIF v_updated_score > 90 THEN
        RAISE WARNING 'Way % has score % which looks like old percentile-based system!', v_way_id, v_updated_score;
    END IF;
END $$;
...
DECLARE
    ...
    v_old_scores_count INTEGER;
BEGIN
    -- Count how many roads were updated
    ...
    RAISE NOTICE 'Update verification: % roads have new scores in test bbox', v_updated_count;
END $$;
...
BEGIN
    ...
    RAISE NOTICE 'Simplified Persona Scoring - Phase 1 Complete';
    RAISE NOTICE 'Test Bbox: 76-78° lon, 12-14° lat (Karnataka region)';
